/* scadenze-2027.js – 04/09/2026, 20:29:10 */
(function(){
  ...
  var PER=20,fil=ALL.slice().sort(function(a,b){return b.valore-a.valore;}),col="valore",dir=-1,page=0;

  /* CSS */
  var s=document.createElement("style");
  s.textContent="#cda-sw *{box-sizing:border-box;}#cda-sw table{width:100%;border-collapse:collapse;font-size:13px;font-family:Arial,sans-serif;}#cda-sw .btn{padding:4px 10px;border:1px solid #1a3a5c;border-radius:4px;background:#fff;color:#1a3a5c;cursor:pointer;font-size:12px;}#cda-sw .btn.on{background:#1a3a5c;color:#fff;}#cda-sw .btn:hover{background:#24527a;color:#fff;}#cda-sw th{background:#1a3a5c;color:#fff;padding:9px 10px;cursor:pointer;user-select:none;white-space:nowrap;text-align:left;}#cda-sw th:hover{background:#24527a;}#cda-sw th.sd:after{content:" ↓";}#cda-sw th.sa:after{content:" ↑";}#cda-sw td{padding:7px 10px;border-bottom:1px solid #e0e0e0;vertical-align:middle;}#cda-sw tr.opt{background:#FFF3CD!important;}#cda-sw tbody tr:not(.opt):nth-child(even){background:#f8f8f8;}#cda-sw img.lg{height:24px;width:24px;object-fit:contain;vertical-align:middle;}#cda-pg{display:flex;gap:6px;flex-wrap:wrap;margin-top:10px;align-items:center;}#cda-pg button{padding:4px 9px;border:1px solid #1a3a5c;border-radius:4px;background:#fff;color:#1a3a5c;cursor:pointer;font-size:12px;}#cda-pg button.on{background:#1a3a5c;color:#fff;}#cda-pg button:disabled{opacity:.35;cursor:default;}.bsi{background:#e8a000;color:#fff;border-radius:3px;padding:2px 7px;font-size:11px;font-weight:bold;}.bno{color:#aaa;font-size:12px;}@media(max-width:500px){#cda-sw td:nth-child(3),#cda-sw th:nth-child(3){display:none;}}";
  ...
  wrap.innerHTML=
    ...
    "<th>Opzione</th>"+
    "</tr></thead><tbody id=\"cda-b\"></tbody></table></div>"+
    "<div id=\"cda-pg\"></div>"+
  ...
  function ltd(sq){
    if(!sq)return"<td>—</td>";
    var u=LOGO[sq];
    return u?"<td><img class=\"lg\" src=\""+u+"\" alt=\""+sq+"\" title=\""+sq+"\"></td>":"<td>"+sq+"</td>";
  }
  ...
  function render(){
    ...
    fil.slice(page*PER,(page+1)*PER).forEach(function(p){
      ...
      h+="<tr"+cls+"><td><strong>"+p.nome+"</strong></td>"+ltd(p.squadra)+"<td style=\"text-align:right;white-space:nowrap\">"+fmt(p.valore)+"</td><td style=\"text-align:center\">"+badge+"</td></tr>";
    });
    ...
    var ph="<button "+(page===0?"disabled":"")+      " data-p=\""+(page-1)+"\">&#8592;</button>";
    for(var i=0;i<pages;i++)
      ph+="<button class=\""+( i===page?"on":"")+"\">"+( i+1)+"</button>";
    ...
    pag.querySelectorAll("button[data-p]").forEach(function(btn){
      btn.addEventListener("click",function(){cdaG(parseInt(this.getAttribute("data-p")));});
    });
    pag.querySelectorAll("button:not([data-p]):not([disabled])").forEach(function(btn,i){
      btn.addEventListener("click",function(){cdaG(i);});
    });
  }
